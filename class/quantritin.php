    ...
    function thongtinuser ($u,$p) {
     ...
     function stripUnicode($str){
         ...
         $unicode = array(
            ...
            'A'=>'Á|À|Ả|Ã|Ạ|Ă|Ắ|Ằ|Ẳ|Ẵ|Ặ| |Ấ|Ầ|Ẩ|Ẫ|Ậ',
            'd'=>'đ', 'D'=>'Đ',
         ...
         foreach($unicode as $khondau=>$codau){
             $arr= explode("|", $codau);
        ...
        function TheLoai_Sua($idTL,$TenTL,$TenTL_KD, $ThuTu, $AnHien, $lang ){
            ...
            settype($ThuTu,"int");
            settype($AnHien,"int");
            $sql="UPDATE theloai SET TenTL='$TenTL',TenTL_KhongDau='$TenTL_KD', ThuTu=$ThuTu, AnHien=$AnHien, lang='$lang' WHERE idTL=$idTL" ;
            $kq = $this->db->query($sql);
            if(!$kq) die($this->db->error);
        }
        
        function TheLoai_KiemTra($idTL){
        ...
        function LoaiTin_Them($Ten,$Ten_KD, $ThuTu, $AnHien, $idTL, $lang){
            ...
            $TenLT_KD=$this->db->escape_string(trim(strip_tags($Ten_KD)));
            if ($Ten_KD=="") $Ten_KD = $this->changeTitle($Ten);
            $lang = $this->db->escape_string(trim(strip_tags($lang)));
            settype($ThuTu,"int");
            settype($AnHien,"int");
            settype($idTL,"int");
            $sql="INSERT INTO loaitin SET Ten='$Ten',Ten_KhongDau='$Ten_KD', ThuTu=$ThuTu, AnHien=$AnHien, idTL = $idTL, lang='$lang'" ;
            $kq = $this->db->query($sql);
            if(!$kq) die($this->db->error);
        ...
        function LoaiTinTrongTheLoai($idTL) {
            $sql="SELECT idLT, Ten FROM loatin WHERE idTL=$idTL ORDER BY ThuTu";
            $kq = $this->db->query($sql); 
            if(!$kq) die($this->db->error);
            return $kq;
        }
        ...
        function Tin_Them($TD, $TD_KD, $TT, $Ngay, $AnHien, $TNB, $urlHinh, $ND, $idTL, $idLT, $tags, $lang){
            ...
            if($TD_KD=="") $TD_KD=$this->changeTitle($TD_KD);
            ...
            settype($AnHien,"int"); settype($TNB,"int"); settype($idTL,"int"); settype($idLT,"int");
            $sql="INSERT INTO tin SET TieuDe='$TD', TomTat='$TT', idTL=$idTL, idLT=$idLT, TieuDe_KhongDau='$TD_KD', Ngay='$Ngay', AnHien=$AnHien, TinNoiBat=$TNB, urlHinh='$urlHinh', Content='$ND', tags='$tags', lang='$lang', idUser=$idUser";
            $kq=$this->db->query($sql);
            if(!$kq) die($this->db->error);
        }
        ...
        function Tin_Sua($TD, $TD_KD, $TT, $Ngay, $AnHien, $TNB, $urlHinh, $ND, $idTL, $idLT, $idTin, $tags, $lang){
            ...
            settype($AnHien,"int"); settype($TNB,"int"); settype($idTL,"int"); settype($idLT,"int");
            $sql="UPDATE tin 
            SET TieuDe='$TD', TomTat='$TT', idTL=$idTL, idLT=$idLT,
             TieuDe_KhongDau='$TD_KD', Ngay='$Ngay', AnHien=$AnHien, 
             TinNoiBat=$TNB, urlHinh='$urlHinh', Content='$ND', tags='$tags', 
             lang='$lang', idLT=$idLT 
             WHERE idTin=$idTin";
            $kq=$this->db->query($sql);
            if(!$kq) die($this->db->error);
        ...
        function ykien_KiemTra($idTin) {
            settype($idTin, 'int');
            $sql = "SELECT * FROM ykien WHERE idTin = $idTin";
            $kq = $this->db->query($sql); 
            if(!$kq) die($this->db->error);
            return $kq; 
        }
